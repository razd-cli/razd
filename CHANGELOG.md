...
- **Mise tool name validation**: Fixed overly restrictive validation that rejected valid mise backend-prefixed tool names ([#19](https://github.com/razd-cli/razd/issues/19))
  - Now supports scoped npm packages: `npm:@fission-ai/openspec`, `npm:@babel/cli`
  - Now supports package managers with extras: `pipx:package[extra1,extra2]`
  - Now supports repository backends: `aqua:owner/repo`, `github:jdx/mise`, `ubi:sharkdp/fd`
  - Now supports go module paths: `go:github.com/owner/repo/cmd/tool`
  - HTTP backend URLs are now accepted without validation
  - Standalone tool names still require alphanumeric, hyphens, and underscores only
  - Added 23 comprehensive unit tests for all backend types
